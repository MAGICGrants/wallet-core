...
BrandPalette _probe() {
  ...
  return BrandPalette(
    paper: next(), card: next(), surfaceSunken: next(), surfaceTinted: next(),
    surfaceMuted: next(), hairline: next(), border: next(), borderStrong: next(),
    inputBorder: next(), frameEdge: next(), ink: next(), inverseSurface: next(),
    inkMuted: next(), inkFaint: next(), inkDisabled: next(), primary: next(),
    primaryDeep: next(), success: next(), successBg: next(), warning: next(),
    warningBg: next(), error: next(), errorBg: next(), purple: next(), blue: next(),
    orange: next(), electrum: next(), surfaceAccent: next(), purpleBg: next(),
    blueBg: next(), orangeBg: next(), electrumBg: next(),
  );
}
